...
class Reviews_Cron {
    ...
    public function register() {
        add_action(Plugin::SLG . '_revupd_schedule', array($this, 'update_schedule'));

        $revupd_cron = get_option(Plugin::SLG . '_revupd_cron');
        $next_cron_run = wp_next_scheduled(Plugin::SLG . '_revupd_schedule');

        if ($revupd_cron !== '0' && !$next_cron_run) {
            $start_at = rand(0, 60 * 60 * 12);
            wp_schedule_event($start_at, 'daily', Plugin::SLG . '_revupd_schedule');
        } elseif ($next_cron_run) {
            update_option(Plugin::SLG . '_revupd_cron_timeout', $next_cron_run - time());
        }
    }
    ...
    public function update_schedule() {
                ...
                if ($feed != false && strlen($feed->post_content) > 0) {
                    ...
                    if ($json->connections && count($json->connections) > 0) {
                        ...
                        foreach ($json->connections as $conn) {
                            ...
                            // Create pair 'pid:lang:local_img' to update it
                            ...
                            if (!in_array($args_key, $feed_conn_cache)) {
                                $this->connect_google->refresh_reviews($args);
        ...
        update_option(Plugin::SLG . '_revupd_cron_log', 'Executed at ' . $now . ' in ' . $end_time . 'ms for feeds: ' . implode(", ", $feed_updated_ids));
    }

    public function deactivate() {
        $next_scheduled = wp_next_scheduled(Plugin::SLG . '_revupd_schedule');
        if ($next_scheduled) {
            wp_unschedule_event($next_scheduled, Plugin::SLG . '_revupd_schedule');
